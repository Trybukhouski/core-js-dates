...
function getNextFriday(date) {
  const nextFriday = (diff) => new Date(date.setDate(date.getDate() + diff));
  const diff = 5 - date.getDay();
  if (diff) {
    return nextFriday(diff > 0 ? diff : 7 - Math.abs(diff));
  }
  return nextFriday(7);
}
...
function isDateInPeriod(date, period) {
  const [start, end] = Object.values(period).map((el) => new Date(el));
...
function formatDate(date) {
  ...
  return `${curDate.getUTCMonth() + 1}/${curDate.getUTCDate()}/${curDate.getUTCFullYear()}, ${curDate.toLocaleTimeString('en-US', { timeZone: 'UTC' })}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  let currentDay = firstweekdayInMonth;
  ...
  for (let i = 0; i < daysPerMonth; i += 1) {
    ...
    currentDay = setNextWeekDay(currentDay);
  }
...
function getWeekNumberByDate(date) {
  ...
  let i = 0;
  while (i <= month) {
    for (let ii = 1; ii <= new Date(year, i, 0).getDate(); ii += 1) {
      if (i === month && ii > day) break;
      if (new Date(year, i, ii).getDay() === 1) count += 1;
    }
    i += 1;
  }
